-- Keeping the cursor centered.
vim.keymap.set("n", "<C-d>", "<C-d>zz", { desc = "Scroll downwards" })
vim.keymap.set("n", "<C-u>", "<C-u>zz", { desc = "Scroll upwards" })
vim.keymap.set("n", "n", "nzzzv", { desc = "Next result" })
vim.keymap.set("n", "N", "Nzzzv", { desc = "Previous result" })

-- Indent while remaining in visual mode.
vim.keymap.set("v", "<", "<gv")
vim.keymap.set("v", ">", ">gv")

-- Formatting.
vim.keymap.set("n", "gQ", "mzgggqG`z<cmd>delmarks z<cr>zz", { desc = "Format buffer" })

-- Make U opposite to u.
vim.keymap.set("n", "U", "<C-r>", { desc = "Redo" })

-- Open the package manager.
vim.keymap.set("n", "<leader>L", "<cmd>Lazy<cr>", { desc = "Lazy" })

-- Restart Neovim.
vim.keymap.set("n", "<leader>R", "<cmd>restart<cr>", { desc = "Restart Neovim" })

-- Switch between windows.
vim.keymap.set("n", "<C-h>", "<C-w>h", { desc = "Move to the left window", remap = true })
vim.keymap.set("n", "<C-j>", "<C-w>j", { desc = "Move to the bottom window", remap = true })
vim.keymap.set("n", "<C-k>", "<C-w>k", { desc = "Move to the top window", remap = true })
vim.keymap.set("n", "<C-l>", "<C-w>l", { desc = "Move to the right window", remap = true })

-- Tab navigation.
vim.keymap.set("n", "<leader>tc", "<cmd>tabclose<cr>", { desc = "Close tab page" })
vim.keymap.set("n", "<leader>tn", "<cmd>tab split<cr>", { desc = "New tab page" })
vim.keymap.set("n", "<leader>to", "<cmd>tabonly<cr>", { desc = "Close other tab pages" })

-- Poweful <esc>.
vim.keymap.set({ "i", "s", "n" }, "<esc>", function()
  -- if require('luasnip').expand_or_jumpable() then
  --     require('luasnip').unlink_current()
  -- end
  vim.cmd("noh")
  return "<esc>"
end, { desc = "Escape, clear hlsearch, and stop snippet session", expr = true })

-- Mark management.
vim.keymap.set("c", "dm", "delmarks", { desc = "Delete marks" })

-- Enter command mode quickly using `;`.
vim.keymap.set("n", ";", ":", { desc = "Enter command mode" })

-- Yank to the system clipboard.
vim.keymap.set({ "i", "n", "v" }, "<C-y>", '"*y', { desc = "Yank to system clipboard" })

-- Paste from the system clipboard.
vim.keymap.set({ "i", "n", "v" }, "<leader>p", '<Esc>"*p', { desc = "Paste from system clipboard" })

-- Move line up
vim.keymap.set("n", "<A-Up>", ":m .-2<CR>==", { desc = "Move line up" })
vim.keymap.set("i", "<A-Up>", "<Esc>:m .-2<CR>==gi", { desc = "Move line up" })
vim.keymap.set("v", "<A-Up>", ":m '<-2<CR>gv=gv", { desc = "Move selection up" })

-- Move line down
vim.keymap.set("n", "<A-Down>", ":m .+1<CR>==", { desc = "Move line down" })
vim.keymap.set("i", "<A-Down>", "<Esc>:m .+1<CR>==gi", { desc = "Move line down" })
vim.keymap.set("v", "<A-Down>", ":m '>+1<CR>gv=gv", { desc = "Move selection down" })

-- Copy line up
vim.keymap.set("n", "<C-A-Up>", ":t .-1<CR>", { desc = "Copy line up" })
vim.keymap.set("i", "<C-A-Up>", "<Esc>:t .-1<CR>gi", { desc = "Copy line up" })
vim.keymap.set("v", "<C-A-Up>", ":t '<-1<CR>gv=gv", { desc = "Copy selection up" })

-- Copy line down
vim.keymap.set("n", "<C-A-Down>", ":t .+0<CR>", { desc = "Copy line down" })
vim.keymap.set("i", "<C-A-Down>", "<Esc>:t .+0<CR>gi", { desc = "Copy line down" })
vim.keymap.set("v", "<C-A-Down>", ":t '>+0<CR>gv=gv", { desc = "Copy selection down" })
